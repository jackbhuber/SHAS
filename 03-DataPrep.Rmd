...
all <- subset (all, select = -c(Ext_Avg:Global_Avg)) # nix these columns for now
...
1. How many data are missing? Obviously the more data are missing, the larger the barrier more this problem poses to analysis.
...
```{r vim-plots, fig.cap='Missing Data 1'}
...
The important question with missing data is whether the missingness is random or systematic. The appropriate first step is therefore to test whether the data are Missing Completely At Random (MCAR) [@McKnightEtAl2007]. 
